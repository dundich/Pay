; (function ($) {
    ...
    // Service URL
    $.kladr.url = 'http://localhost/Ait.Web.Kladr/api/kladr';//'http://kladr-api.ru/api.php';
        ...
        function kladr(input, param1, param2) {
            ...
            var defaultOptions = {
                ...
                labelFormat: function (obj, query) {
                    ...
                    if (start >= 0) {
                        start = start + 2;
                        label = label.substr(start, label.length - start);
                    }
                    ...
                    for (var i = 0, len = split_query.length; i < len; i++) {

                        squery = split_query[i];
                        sstart = label.toLowerCase().indexOf(squery);

                        if (sstart >= 0) {
                            label = label.substr(0, sstart)
                              + '<strong>' + label.substr(sstart, squery.length) + '</strong>'
                              + label.substr(sstart + squery.length, label.length - sstart - squery.length);
                        }
                    }
            ...
            function create() {
                ...
                if (!container.length) {
                    container = $('<div class="kladr_autocomplete"></div>').appendTo('body');
                }
                ...
                spinner = $('<div class="spinner kladr_autocomplete_' + inputName + '_spinner" class="spinner" style="display: none;"><i class="fa fa-spinner fa-pulse fa-2x fa-fw"></i><span class="sr-only">Loading...</span></div>');
            ...
            function validate() {
                switch (options.type) {
                    ...
                    case $.kladr.type.street:
                        if (options.parentType != $.kladr.type.city && options.parentType != $.kladr.type.region) {
